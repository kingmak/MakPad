...
def BodySelectAll(event, ObjectName):
    end = len(ObjectName.get(1.0, END)[:-1])
    #ObjectName.select_range (0, end)
    return 'break'
...
def MainMenus():
    ...
    Editmenu.add_command(label = 'Select All', command = lambda: Body.event_generate('<<SelectAll>>'))
...
w, h = (root.winfo_screenwidth() - 20), (root.winfo_screenheight() - 118)
...
TimeStampInsert = Button(root, fg = 'orange', text = 'Insert Time Stamp', command = InsertTimeStamp).place(x = 950, y = 4)
CommentInsert = Button(root, fg = 'orange', text = 'Insert Comment Lines', command = InsertUserBio).place(x = 1215, y = 4)

RightScrollbar = Scrollbar(root)
RightScrollbar.pack(side = RIGHT, fill = Y)

Body = Text(root, width = 132, height = 33, fg = 'orange', selectbackground = 'white', highlightcolor = 'red', wrap = WORD, undo = True, yscrollcommand = RightScrollbar.set)
Body.bind('<Button-3>', lambda event: EntryPopupMenu(event, Body))
Body.bind('<Control-Key-a>', lambda: Body.event_generate('<<SelectAll>>'))
